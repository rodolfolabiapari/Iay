...
// Fourier Functions
////////////////////////////////////////////////////////////////
//void fourn(float data[nn1 * nn2 * nn3], unsigned long nn[4], int ndim, int
  //isign)
...
/*
 * This procedure gets the FIRST FPGA Platform and returns its context
 */
cl_context initialize_opencl(cl_platform_id * platform_id,
    cl_device_id * device_id)
{
  ...
  // Gets all the num_platforms and its IDs
  //* platform_id = get_platforms(num_platforms, &status);
  // Gets the FPGA Platform
  * platform_id = find_platform(PLATFORM_NAME, &status);
  test_error(status, "ERROR: Unable to find the OpenCL FPGA platform.\n",
    &cleanup);

  // print some info on the selected platform
  {
    char * platform_info = get_platform_info(* platform_id, CL_PLATFORM_NAME);
    printf("%-20s = %s\n", "CL_PLATFORM_NAME", platform_info);
    platform_info = get_platform_info(* platform_id, CL_PLATFORM_VENDOR);
    printf("%-20s = %s\n", "CL_PLATFORM_VENDOR", platform_info);
    platform_info = get_platform_info(* platform_id, CL_PLATFORM_VERSION);
  ...
  //  &num_devices, &status);
  * devices_id = get_devices(* platform_id, CL_DEVICE_TYPE_ALL,
    &num_devices, &status);
...
*/
  ...
  status = clSetKernalArg(kernel, 1, sizeof(cl_float), (void *) &c_host);
  status = clSetKernalArg(kernel, 2, sizeof(cl_int),   (void *) &NUM_ELEMENTS);
